...
export default function EscalationRulesIndex() {
  ...
  const formatTimeTrigger = (rule: EscalationRule) => {
    ...
    const triggerLabel = timeTriggerTypes[rule.time_trigger_type] || rule.time_trigger_type;

    return `${timeStr} after ${triggerLabel}`;
  };
                ...
                <TableBody>
                  {rules.data.map((rule) => (
                    <TableRow key={rule.id}>
                      ...
                      <TableCell>
                        <p className="text-sm">{formatTimeTrigger(rule)}</p>
                      </TableCell>
                      <TableCell>
